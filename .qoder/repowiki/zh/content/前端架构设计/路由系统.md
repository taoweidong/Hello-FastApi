...
- [web/src/router/modules/home.ts](file://web/src/router/modules/home.ts)
- [web/src/router/modules/nested.ts](file://web/src/router/modules/nested.ts)
- [web/src/router/modules/error.ts](file://web/src/router/modules/error.ts)
- [web/src/router/modules/form.ts](file://web/src/router/modules/form.ts)
- [web/src/router/modules/table.ts](file://web/src/router/modules/table.ts)
...
B --> C["模块化静态路由 modules/*.ts<br/>home/nested/error/form/table 等"]
...
- [web/src/router/modules/home.ts:6-27](file://web/src/router/modules/home.ts#L6-L27)
- [web/src/router/modules/nested.ts:4-81](file://web/src/router/modules/nested.ts#L4-L81)
- [web/src/router/modules/error.ts:4-39](file://web/src/router/modules/error.ts#L4-L39)
- [web/src/router/modules/form.ts:4-22](file://web/src/router/modules/form.ts#L4-L22)
- [web/src/router/modules/table.ts:4-46](file://web/src/router/modules/table.ts#L4-L46)
...
### 路由定义与模块化组织
- 静态路由模块：按功能域拆分，如 home、nested、error、form、table 等，每个模块导出路由配置对象，包含 path、name、redirect、meta、children 等。
- remaining 路由：集中放置登录、重定向、全屏错误页等不受菜单系统控制的路由。
- 路由枚举：通过 enums.ts 统一维护各模块的排序权重 rank，便于后端返回时按 rank 排序。

```mermaid
flowchart TD
Start(["开始"]) --> LoadModules["自动导入 modules/*.ts"]
...
```mermaid
flowchart TD
Static["静态路由模块"] --> Glob["import.meta.glob 自动导入"]
...
### 嵌套路由、重定向与路由别名
- 嵌套路由：nested 模块演示三级及以上嵌套，最终拍成两级路由以支持 keep-alive。
...
**图示来源**
- [web/src/router/modules/nested.ts:6-81](file://web/src/router/modules/nested.ts#L6-L81)
- [web/src/router/utils.ts:317-343](file://web/src/router/utils.ts#L317-L343)
...
### 路由模块化组织策略与路由枚举定义
- 按功能域划分模块：components、form、table、nested、error 等，每个模块自包含路由定义与视图组件。
...
## 依赖关系分析
- 路由入口依赖：modules/*.ts、remaining.ts、utils.ts、permission store。
...
router_index_ts --> modules_ts["router/modules/*.ts"]
...
## 结论
本路由系统通过“静态 + 动态”双轨设计、模块化组织与完善的工具链，实现了灵活的权限控制、良好的用户体验与可维护性。建议在实际项目中：
- 统一使用枚举管理排序与权限；
- 严格区分静态与动态路由职责；
