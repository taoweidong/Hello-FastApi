...
participant Meta as "元数据(SQLModel.metadata)"
...
- 角色-权限（多对多）
  - 关联表：role_permissions，使用显式的 RolePermissionLink 模型作为中间表，role_id 与 permission_id 外键均设置删除级联，保证角色或权限删除后关联自动清理。
  - 双向关系：Role.permissions 与 Permission.roles 通过中间表建立多对多，使用 link_model 参数指定 RolePermissionLink 模型。
- 用户-用户角色（一对多）
...
**更新** 多对多关系现在使用显式的 RolePermissionLink 模型作为中间表，而不是之前的 Table 对象，提供了更好的类型安全性和查询灵活性。

章节来源
- [models.py:17-27](file://src/infrastructure/database/models.py#L17-L27)
- [models.py:67-89](file://src/infrastructure/database/models.py#L67-L89)
- [models.py:95-116](file://src/infrastructure/database/models.py#L95-L116)
...
章节来源
- [models.py:22-27](file://src/infrastructure/database/models.py#L22-L27)
- [models.py:128-133](file://src/infrastructure/database/models.py#L128-L133)
...
章节来源
- [models.py:38-41](file://src/infrastructure/database/models.py#L38-L41)
- [models.py:102](file://src/infrastructure/database/models.py#L102)
- [models.py:156](file://src/infrastructure/database/models.py#L156)
- [models.py:17-27](file://src/infrastructure/database/models.py#L17-L27)
- [models.py:128-133](file://src/infrastructure/database/models.py#L128-L133)
...
- 典型查询模式：
  ...
  - 角色-权限：通过 RolePermissionLink 中间表查询某角色的全部权限。
  - 用户-权限：通过 RolePermissionLink 和用户角色表联合查询某用户的全部权限。
- 查询语法更新：使用 one_or_none() 替代之前的 scalar_one_or_none()，提供更清晰的单结果查询语义。
- 建议：在高频读取场景中优先使用 selectinload；对只读场景可考虑使用 joinedload 或仅加载必要字段；对复杂筛选组合，结合复合索引与分页。

**更新** 查询语法从 scalar_one_or_none 改为 one_or_none，更符合 SQLModel 的查询模式，提供更明确的单结果查询语义。

章节来源
- [user_repository.py:17-27](file://src/infrastructure/repositories/user_repository.py#L17-L27)
- [rbac_repository.py:17-23](file://src/infrastructure/repositories/rbac_repository.py#L17-L23)
...
- 示例路径（不展示具体代码，仅给出路径）：
  ...
  - 获取角色的所有权限（通过 RolePermissionLink 中间表）：[rbac_repository.py:106-112](file://src/infrastructure/repositories/rbac_repository.py#L106-L112)
  - 获取用户的全部权限（通过 RolePermissionLink 与用户角色表）：[rbac_repository.py:114-122](file://src/infrastructure/repositories/rbac_repository.py#L114-L122)
- 最佳实践：
...
**更新** 多对多查询现在使用 RolePermissionLink 模型进行连接，提供更清晰的中间表访问方式。
...
**更新** 由于使用了显式的 RolePermissionLink 模型，迁移时需要特别注意该模型的创建和删除，以及 link_model 参数的正确配置。
...
- 查询语法问题：如果遇到 one_or_none() 相关错误，确认使用的是 SQLModel 的查询语法而非 SQLAlchemy 的 scalar_one_or_none()。

**更新** 新增了查询语法问题的排查指导，特别是 one_or_none() 与 scalar_one_or_none() 的区别。
...
## 结论
本项目通过清晰的分层与接口约束，实现了稳定的数据库关系映射与查询优化。多对多关系通过显式的 RolePermissionLink 模型与 CASCADE 级联策略保证数据一致性；索引与关系加载策略有效降低了查询成本。配合 Alembic 的迁移能力，可在演进过程中安全地调整关系与结构。SQLModel 的引入提供了更好的类型安全性和查询体验，one_or_none() 语法比之前的 scalar_one_or_none() 更加直观。建议在生产环境中持续监控慢查询与索引命中率，并根据业务增长动态调整索引与加载策略。
