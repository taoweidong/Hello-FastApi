...
## 更新摘要
**变更内容**
- 从SQLAlchemy迁移到SQLModel的架构重构
- 数据传输对象(DTO)统一使用SQLModel而非Pydantic
- 异步连接管理和会话处理机制更新
- 仓储实现完全重构以适配SQLModel查询语法
- 统一ORM和数据验证能力的集成
...
**更新** 本版本反映了从 SQLAlchemy 到 SQLModel 的架构迁移，实现了 ORM 与数据验证的统一，使用 SQLModel 的异步连接管理和新的仓储实现模式。

## 项目结构
项目采用分层架构，数据模型位于基础设施层（infrastructure）的 SQLModel ORM 中，通过仓储层（repositories）提供数据访问能力。模型使用 UUID 作为主键，支持异步数据库操作，并集成了 Pydantic 数据验证功能。

```mermaid
graph TB
subgraph "基础设施层(infrastructure)"
DBM["models.py 模型定义<br/>SQLModel + Pydantic 统一ORM"]
CONN["connection.py 异步连接管理"]
END
subgraph "仓储层(repositories)"
RU["UserRepository 异步实现"]
RR["RoleRepository 异步实现"]
PR["PermissionRepository 异步实现"]
END
subgraph "领域层(domain)"
UI["UserRepositoryInterface"]
RI["RoleRepositoryInterface"]
PI["PermissionRepositoryInterface"]
END
DBM --> CONN
CONN --> RU
CONN --> RR
...
- [src/infrastructure/repositories/user_repository.py:1-57](file://src/infrastructure/repositories/user_repository.py#L1-L57)
...
**章节来源**
- [src/infrastructure/database/models.py:33-171](file://src/infrastructure/database/models.py#L33-L171)
...
B --> C["创建所有表<br/>SQLModel.metadata.create_all"]
C --> D["异步ORM基类 SQLModel<br/>connection.py"]
D --> E["模型定义 models.py<br/>SQLModel + Pydantic"]
E --> F["User 实体(users)"]
...
**图表来源**
- [src/main.py:18-31](file://src/main.py#L18-L31)
...
### 用户实体模型（User）
- 设计要点
  - 使用 UUID 作为主键，长度 36 字符，避免整数溢出与序列化问题
...
**图表来源**
- [src/infrastructure/database/models.py:33-62](file://src/infrastructure/database/models.py#L33-L62)
...
Role "many" --> "many" Permission : "通过 RolePermissionLink 关联"
```

**图表来源**
- [src/infrastructure/database/models.py:67-120](file://src/infrastructure/database/models.py#L67-L120)
...
**图表来源**
- [src/application/services/auth_service.py:20-40](file://src/application/services/auth_service.py#L20-L40)
- [src/infrastructure/repositories/user_repository.py:17-27](file://src/infrastructure/repositories/user_repository.py#L17-L27)
...
ROLE_PERMISSIONS {
string role_id PK,FK(UUID)
string permission_id PK,FK(UUID)
}
...
**图表来源**
- [src/infrastructure/repositories/user_repository.py:17-31](file://src/infrastructure/repositories/user_repository.py#L17-L31)
- [src/infrastructure/repositories/rbac_repository.py:67-71](file://src/infrastructure/repositories/rbac_repository.py#L67-L71)
...
### 数据传输对象（DTO）设计
- 设计要点
  - 所有 DTO 现在使用 SQLModel，继承统一的 ORM 和数据验证能力
  - UserCreateDTO、UserUpdateDTO、UserResponseDTO 提供完整的用户数据验证
  - RBAC DTO 包含角色和权限的创建、更新、响应格式
  - Auth DTO 提供登录和令牌响应的数据结构
- 数据验证
  - 使用 Pydantic 的字段验证器进行输入验证
  - 支持最小长度、最大长度、EmailStr 等验证规则
...
**章节来源**
- [src/application/dto/user_dto.py:1-54](file://src/application/dto/user_dto.py#L1-L54)
...
## 依赖关系分析
- 模型注册
  - models.py 导入所有实体，确保 SQLModel 能在 create_all 时发现并创建表
- 会话与连接
  - connection.py 统一提供异步引擎、会话工厂与 SQLModel 基类
  - main.py 在应用生命周期内调用 init_db()/close_db()
- 仓储接口与实现
  - domain 层定义抽象接口，infrastructure 层提供 SQLModel 实现
  - 接口与实现分离，便于替换与测试
- DTO 集成
  - 所有 DTO 现在使用 SQLModel，与 ORM 模型保持一致的数据结构

```mermaid
graph LR
IF["接口(domain/*.py)"] --> IMPL["实现(infrastructure/repositories/*.py)"]
IMPL --> ORM["SQLModel 模型(models.py)"]
ORM --> DB["数据库"]
MAIN["应用(main.py)"] --> INIT["init_db()"]
INIT --> ORM
DTO["DTO (application/dto/*.py)"] --> ORM
```
...
- SQLModel 优化
  - 使用 SQLModel 的原生查询语法，减少 ORM 层开销
  - 利用 SQLModel 的类型安全特性，避免运行时错误
...
- SQLModel 特有问题
  - 确保所有模型都正确继承 SQLModel(table=True)
  - 检查字段定义中的 sa_column 参数是否正确配置
  - 验证异步会话的生命周期管理
...
## 结论
本数据模型以清晰的领域边界与分层架构为基础，通过 SQLModel 实现了用户、角色、权限与安全规则的完整建模。SQLModel 的统一 ORM 和 Pydantic 数据验证能力提供了更好的开发体验和类型安全性。模型具备良好的扩展性与可维护性，配合仓储层与会话管理机制，能够满足高并发场景下的性能与一致性需求。建议后续引入版本化迁移工具以完善数据库演进管理。

## 附录
- 数据库迁移与版本管理建议
  - 使用 Alembic 进行迁移脚本管理，基于 SQLModel 模型生成初始迁移
...
- SQLModel 最佳实践
  - 充分利用 SQLModel 的类型推断和自动验证
  - 使用统一的模型配置和字段定义规范
  - 结合 Pydantic 的数据验证特性，确保数据完整性