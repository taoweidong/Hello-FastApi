# 数据模型设计

<cite>
**本文档引用的文件**
- [src/infrastructure/database/models.py](file://src/infrastructure/database/models.py)
- [src/infrastructure/database/connection.py](file://src/infrastructure/database/connection.py)
- [src/infrastructure/repositories/user_repository.py](file://src/infrastructure/repositories/user_repository.py)
- [src/infrastructure/repositories/rbac_repository.py](file://src/infrastructure/repositories/rbac_repository.py)
- [src/domain/user/repository.py](file://src/domain/user/repository.py)
- [src/domain/rbac/repository.py](file://src/domain/rbac/repository.py)
- [src/config/settings.py](file://src/config/settings.py)
- [src/main.py](file://src/main.py)
- [src/application/services/auth_service.py](file://src/application/services/auth_service.py)
- [src/domain/auth/token_service.py](file://src/domain/auth/token_service.py)
- [src/infrastructure/cache/redis_client.py](file://src/infrastructure/cache/redis_client.py)
</cite>

## 目录
1. [简介](#简介)
2. [项目结构](#项目结构)
3. [核心组件](#核心组件)
4. [架构总览](#架构总览)
5. [详细组件分析](#详细组件分析)
6. [依赖关系分析](#依赖关系分析)
7. [性能考虑](#性能考虑)
8. [故障排除指南](#故障排除指南)
9. [结论](#结论)
10. [附录](#附录)

## 简介
本文件系统性阐述本项目的数据库模型设计与关系映射，覆盖用户实体、RBAC 权限模型、安全规则模型以及数据库连接与事务处理机制。重点说明：
- 用户实体的字段定义、约束与索引策略
- 角色与权限实体的设计、权限枚举与层级关系
- 用户-角色关联表的多对多实现与级联操作
- 安全相关模型（IP 白黑名单）与会话/令牌管理
- 数据库连接管理、事务处理与生命周期
- 模型间关系映射（一对一、一对多、多对多）
- 数据库迁移与版本管理建议
- 性能优化与查询优化技巧

**更新** 本版本反映了从 Django ORM 到 SQLAlchemy 2.0+ 异步模型的迁移，包括 UUID 主键、异步连接管理和新的仓储实现模式。

## 项目结构
项目采用分层架构，数据模型位于基础设施层（infrastructure）的 SQLAlchemy ORM 中，通过仓储层（repositories）提供数据访问能力。模型使用 UUID 作为主键，支持异步数据库操作。

```mermaid
graph TB
subgraph "基础设施层(infrastructure)"
DBM["models.py 模型定义<br/>UUID主键 + 异步ORM"]
CONN["connection.py 异步连接管理"]
END
subgraph "仓储层(repositories)"
RU["UserRepository 异步实现"]
RR["RoleRepository 异步实现"]
PR["PermissionRepository 异步实现"]
END
subgraph "领域层(domain)"
UI["UserRepositoryInterface"]
RI["RoleRepositoryInterface"]
PI["PermissionRepositoryInterface"]
END
DBM --> CONN
CONN --> RU
CONN --> RR
CONN --> PR
UI --> RU
RI --> RR
PI --> PR
```

**图表来源**
- [src/infrastructure/database/models.py:1-142](file://src/infrastructure/database/models.py#L1-L142)
- [src/infrastructure/database/connection.py:1-52](file://src/infrastructure/database/connection.py#L1-L52)
- [src/infrastructure/repositories/user_repository.py:1-61](file://src/infrastructure/repositories/user_repository.py#L1-L61)
- [src/infrastructure/repositories/rbac_repository.py:1-133](file://src/infrastructure/repositories/rbac_repository.py#L1-L133)
- [src/domain/user/repository.py:1-50](file://src/domain/user/repository.py#L1-L50)
- [src/domain/rbac/repository.py:1-62](file://src/domain/rbac/repository.py#L1-L62)

**章节来源**
- [src/infrastructure/database/models.py:1-142](file://src/infrastructure/database/models.py#L1-L142)
- [src/infrastructure/database/connection.py:1-52](file://src/infrastructure/database/connection.py#L1-L52)
- [src/infrastructure/repositories/user_repository.py:1-61](file://src/infrastructure/repositories/user_repository.py#L1-L61)
- [src/infrastructure/repositories/rbac_repository.py:1-133](file://src/infrastructure/repositories/rbac_repository.py#L1-L133)
- [src/domain/user/repository.py:1-50](file://src/domain/user/repository.py#L1-L50)
- [src/domain/rbac/repository.py:1-62](file://src/domain/rbac/repository.py#L1-L62)

## 核心组件
本节概述各实体模型及其关键属性、约束与索引策略。

- 用户实体（users）
  - 主键：UUID 字符串（36 字符）
  - 唯一索引：username、email
  - 普通索引：username、email（用于加速查找）
  - 字段：用户名、邮箱、加密密码、全名、激活状态、超级用户标记、创建/更新时间戳
  - 关系：与用户-角色关联表的一对多（删除时级联清理）

- 角色实体（roles）
  - 主键：UUID 字符串（36 字符）
  - 唯一索引：name
  - 普通索引：name
  - 字段：名称、描述、创建/更新时间戳
  - 关系：与权限的多对多（通过中间表 role_permissions），与用户-角色关联表的一对多（删除时级联清理）

- 权限实体（permissions）
  - 主键：UUID 字符串（36 字符）
  - 唯一索引：name、codename
  - 字段：名称、编码、描述、资源、动作、创建时间
  - 关系：与角色的多对多（通过中间表 role_permissions）

- 用户-角色关联实体（user_roles）
  - 主键：UUID 字符串（36 字符）
  - 外键：user_id、role_id（均带索引）
  - 字段：分配时间
  - 关系：与 User、Role 双向回溯

- IP 规则实体（ip_rules）
  - 主键：UUID 字符串（36 字符）
  - 索引：ip_address
  - 字段：IP 地址、规则类型（白/黑名单）、原因、启用状态、创建时间、过期时间
  - 用途：黑白名单与限流

**章节来源**
- [src/infrastructure/database/models.py:29-142](file://src/infrastructure/database/models.py#L29-L142)

## 架构总览
下图展示数据模型在系统中的位置与交互流程，包括启动时初始化、会话管理与事务处理。

```mermaid
graph TB
A["应用启动(lifespan)<br/>src/main.py"] --> B["初始化数据库<br/>init_db()"]
B --> C["创建所有表<br/>Base.metadata.create_all"]
C --> D["异步ORM基类 Base<br/>connection.py"]
D --> E["模型定义 models.py<br/>UUID + 异步关系"]
E --> F["User 实体(users)"]
E --> G["Role/Permission 实体(roles/permissions)"]
E --> H["UserRole 关联表(user_roles)"]
E --> I["IPRule 实体(ip_rules)"]
J["请求处理"] --> K["依赖注入 get_db()<br/>异步会话"]
K --> L["读写操作(commit/rollback/close)"]
```

**图表来源**
- [src/main.py:19-32](file://src/main.py#L19-L32)
- [src/infrastructure/database/connection.py:40-52](file://src/infrastructure/database/connection.py#L40-L52)
- [src/infrastructure/database/models.py:1-142](file://src/infrastructure/database/models.py#L1-L142)

**章节来源**
- [src/main.py:19-32](file://src/main.py#L19-L32)
- [src/infrastructure/database/connection.py:1-52](file://src/infrastructure/database/connection.py#L1-L52)
- [src/infrastructure/database/models.py:1-142](file://src/infrastructure/database/models.py#L1-L142)

## 详细组件分析

### 用户实体模型（User）
- 设计要点
  - 使用 UUID 作为主键，长度 36 字符，避免整数溢出与序列化问题
  - username 与 email 均建立唯一索引与普通索引，支持快速登录与检索
  - is_active 控制账户启用状态；is_superuser 提供全局管理员标记
  - created_at/updated_at 使用服务器默认值与更新触发器，保证时间一致性
  - 与 UserRole 的一对多关系，删除用户时级联删除其角色分配记录

```mermaid
classDiagram
class User {
+id : string(UUID)
+username : string
+email : string
+hashed_password : string
+full_name : string?
+is_active : bool
+is_superuser : bool
+created_at : datetime
+updated_at : datetime
}
class UserRole {
+id : string(UUID)
+user_id : string(UUID)
+role_id : string(UUID)
+assigned_at : datetime
}
User "1" --> "many" UserRole : "拥有"
```

**图表来源**
- [src/infrastructure/database/models.py:29-53](file://src/infrastructure/database/models.py#L29-L53)
- [src/infrastructure/database/models.py:106-122](file://src/infrastructure/database/models.py#L106-L122)

**章节来源**
- [src/infrastructure/database/models.py:29-53](file://src/infrastructure/database/models.py#L29-L53)

### 角色与权限实体模型（Role/Permission）
- 设计要点
  - 角色与权限均为独立实体，通过中间表 role_permissions 建立多对多关系
  - 权限实体包含资源与动作字段，便于细粒度授权判断
  - 角色实体维护 created_at/updated_at，支持审计与变更追踪
  - 中间表在删除角色或权限时采用 CASCADE，确保关系完整性

```mermaid
classDiagram
class Role {
+id : string(UUID)
+name : string
+description : string?
+created_at : datetime
}
class Permission {
+id : string(UUID)
+name : string
+codename : string
+description : string?
+resource : string?
+action : string?
+created_at : datetime
}
class role_permissions {
+role_id : string(UUID)
+permission_id : string(UUID)
}
Role "many" --> "many" Permission : "通过 role_permissions 关联"
```

**图表来源**
- [src/infrastructure/database/models.py:58-104](file://src/infrastructure/database/models.py#L58-L104)

**章节来源**
- [src/infrastructure/database/models.py:58-104](file://src/infrastructure/database/models.py#L58-L104)

### 用户-角色关联表（UserRole）
- 设计要点
  - 作为用户与角色的关联实体，承载分配时间等元数据
  - user_id 与 role_id 均建立索引，提升查询效率
  - 外键删除策略为 CASCADE，保障级联清理
  - 与 User/Role 的双向关系，支持从任一侧导航

```mermaid
classDiagram
class UserRole {
+id : string(UUID)
+user_id : string(UUID)
+role_id : string(UUID)
+assigned_at : datetime
}
class User
class Role
UserRole --> User : "属于"
UserRole --> Role : "属于"
```

**图表来源**
- [src/infrastructure/database/models.py:106-122](file://src/infrastructure/database/models.py#L106-L122)

**章节来源**
- [src/infrastructure/database/models.py:106-122](file://src/infrastructure/database/models.py#L106-L122)

### 安全相关数据模型（IPRule）
- 设计要点
  - 支持白名单与黑名单两种规则类型
  - ip_address 建立索引，便于快速匹配
  - is_active 控制规则启用状态；expires_at 支持临时封禁
  - 适合与网关或中间件配合实现访问控制

```mermaid
classDiagram
class IPRule {
+id : string(UUID)
+ip_address : string
+rule_type : string
+reason : string?
+is_active : bool
+created_at : datetime
+expires_at : datetime?
}
```

**图表来源**
- [src/infrastructure/database/models.py:127-142](file://src/infrastructure/database/models.py#L127-L142)

**章节来源**
- [src/infrastructure/database/models.py:127-142](file://src/infrastructure/database/models.py#L127-L142)

### 会话管理、令牌存储与审计
- 会话管理
  - 异步数据库引擎与会话工厂，支持连接池预检与自动回滚
  - 会话在依赖注入中按请求生成，结束后自动提交或回滚并关闭
- 令牌管理
  - 使用 JWT 令牌服务进行访问/刷新令牌的生成与校验
  - 令牌负载包含用户标识与类型，便于鉴权与区分访问范围
- 审计日志
  - 用户与角色实体均具备创建/更新时间戳，可用于审计追踪

```mermaid
sequenceDiagram
participant Client as "客户端"
participant App as "应用服务(AuthService)"
participant Repo as "UserRepository"
participant DB as "数据库"
Client->>App : "POST /api/v1/auth/login"
App->>Repo : "按用户名查询用户"
Repo->>DB : "SELECT * FROM users WHERE username=..."
DB-->>Repo : "返回用户记录"
Repo-->>App : "用户对象"
App->>App : "校验密码与账户状态"
App->>App : "生成访问/刷新令牌"
App-->>Client : "返回令牌"
```

**图表来源**
- [src/application/services/auth_service.py:13-33](file://src/application/services/auth_service.py#L13-L33)
- [src/infrastructure/repositories/user_repository.py:17-30](file://src/infrastructure/repositories/user_repository.py#L17-L30)
- [src/domain/auth/token_service.py:9-32](file://src/domain/auth/token_service.py#L9-L32)

**章节来源**
- [src/infrastructure/database/connection.py:27-38](file://src/infrastructure/database/connection.py#L27-L38)
- [src/application/services/auth_service.py:13-33](file://src/application/services/auth_service.py#L13-L33)
- [src/domain/auth/token_service.py:9-32](file://src/domain/auth/token_service.py#L9-L32)

### 数据库连接管理与事务处理
- 连接与会话
  - 异步引擎与会话工厂，开启 pool_pre_ping 以提升连接稳定性
  - Base 作为声明式基类，统一模型注册入口
- 生命周期
  - 应用启动时调用 init_db() 初始化表结构
  - 应用关闭时 dispose 引擎释放资源
- 事务
  - 依赖注入中捕获异常并执行回滚，确保一致性
  - 成功路径自动提交，失败路径回滚并抛出异常

```mermaid
flowchart TD
Start(["请求开始"]) --> GetSession["获取异步会话"]
GetSession --> TryOp["执行数据库操作"]
TryOp --> Commit{"是否成功?"}
Commit --> |是| DoCommit["提交事务"]
Commit --> |否| DoRollback["回滚事务"]
DoCommit --> Close["关闭会话"]
DoRollback --> Raise["抛出异常"]
Raise --> Close
Close --> End(["请求结束"])
```

**图表来源**
- [src/infrastructure/database/connection.py:27-38](file://src/infrastructure/database/connection.py#L27-L38)
- [src/main.py:19-32](file://src/main.py#L19-L32)

**章节来源**
- [src/infrastructure/database/connection.py:1-52](file://src/infrastructure/database/connection.py#L1-L52)
- [src/main.py:19-32](file://src/main.py#L19-L32)

### 模型间关系映射
- 一对一：用户与用户-角色关联（通过 UserRole 间接体现）
- 一对多：用户 → 角色分配；角色 → 权限分配
- 多对多：角色 ↔ 权限（通过中间表 role_permissions）
- 级联策略：用户删除时级联删除其角色分配；角色/权限删除时级联删除中间表记录

```mermaid
erDiagram
USERS {
string id PK(UUID)
string username UK
string email UK
string hashed_password
string full_name
boolean is_active
boolean is_superuser
timestamp created_at
timestamp updated_at
}
ROLES {
string id PK(UUID)
string name UK
string description
timestamp created_at
}
PERMISSIONS {
string id PK(UUID)
string name UK
string codename UK
string resource
string action
timestamp created_at
}
USER_ROLES {
string id PK(UUID)
string user_id FK(UUID)
string role_id FK(UUID)
timestamp assigned_at
}
ROLE_PERMISSIONS {
string role_id PK,FK(UUID)
string permission_id PK,FK(UUID)
}
USERS ||--o{ USER_ROLES : "拥有"
ROLES ||--o{ USER_ROLES : "被分配"
ROLES ||--o{ ROLE_PERMISSIONS : "拥有"
PERMISSIONS ||--o{ ROLE_PERMISSIONS : "被授予"
```

**图表来源**
- [src/infrastructure/database/models.py:29-142](file://src/infrastructure/database/models.py#L29-L142)

**章节来源**
- [src/infrastructure/database/models.py:29-142](file://src/infrastructure/database/models.py#L29-L142)

### 查询与仓储实现
- 用户仓储
  - 支持按 id/username/email 查询，使用 selectinload 预加载角色关系
  - 提供分页查询、创建、更新、删除与计数
- 角色与权限仓储
  - 角色：按 id/name 查询并预加载权限；支持用户角色查询
  - 权限：按 id/codename 查询；支持角色权限与用户权限查询
- 查询优化
  - 使用 selectinload 减少 N+1 查询
  - 在高频查询字段上建立索引（username、email、role_id、permission_id）

```mermaid
sequenceDiagram
participant Svc as "应用服务"
participant Repo as "UserRepository"
participant DB as "数据库"
Svc->>Repo : "get_by_username(username)"
Repo->>DB : "SELECT * FROM users WHERE username=...<br/>JOIN user_roles"
DB-->>Repo : "用户 + 角色列表"
Repo-->>Svc : "User 对象"
```

**图表来源**
- [src/infrastructure/repositories/user_repository.py:17-35](file://src/infrastructure/repositories/user_repository.py#L17-L35)
- [src/infrastructure/repositories/rbac_repository.py:68-76](file://src/infrastructure/repositories/rbac_repository.py#L68-L76)

**章节来源**
- [src/infrastructure/repositories/user_repository.py:1-61](file://src/infrastructure/repositories/user_repository.py#L1-L61)
- [src/infrastructure/repositories/rbac_repository.py:1-133](file://src/infrastructure/repositories/rbac_repository.py#L1-L133)

## 依赖关系分析
- 模型注册
  - models.py 导入所有实体，确保 SQLAlchemy 能在 create_all 时发现并创建表
- 会话与连接
  - connection.py 统一提供异步引擎、会话工厂与 Base 基类
  - main.py 在应用生命周期内调用 init_db()/close_db()
- 仓储接口与实现
  - domain 层定义抽象接口，infrastructure 层提供 SQLAlchemy 实现
  - 接口与实现分离，便于替换与测试

```mermaid
graph LR
IF["接口(domain/*.py)"] --> IMPL["实现(infrastructure/repositories/*.py)"]
IMPL --> ORM["SQLAlchemy 模型(models.py)"]
ORM --> DB["数据库"]
MAIN["应用(main.py)"] --> INIT["init_db()"]
INIT --> ORM
```

**图表来源**
- [src/domain/user/repository.py:8-50](file://src/domain/user/repository.py#L8-L50)
- [src/domain/rbac/repository.py:8-62](file://src/domain/rbac/repository.py#L8-L62)
- [src/infrastructure/repositories/user_repository.py:1-61](file://src/infrastructure/repositories/user_repository.py#L1-L61)
- [src/infrastructure/repositories/rbac_repository.py:1-133](file://src/infrastructure/repositories/rbac_repository.py#L1-L133)
- [src/infrastructure/database/models.py:1-142](file://src/infrastructure/database/models.py#L1-L142)
- [src/main.py:19-32](file://src/main.py#L19-L32)

**章节来源**
- [src/domain/user/repository.py:8-50](file://src/domain/user/repository.py#L8-L50)
- [src/domain/rbac/repository.py:8-62](file://src/domain/rbac/repository.py#L8-L62)
- [src/infrastructure/repositories/user_repository.py:1-61](file://src/infrastructure/repositories/user_repository.py#L1-L61)
- [src/infrastructure/repositories/rbac_repository.py:1-133](file://src/infrastructure/repositories/rbac_repository.py#L1-L133)
- [src/infrastructure/database/models.py:1-142](file://src/infrastructure/database/models.py#L1-L142)
- [src/main.py:19-32](file://src/main.py#L19-L32)

## 性能考虑
- 索引策略
  - 在高频过滤字段（如 username、email、role_id、permission_id、ip_address）建立索引
  - 复合索引用于常见查询模式（如用户-角色联合查询）
- 查询优化
  - 使用 selectinload 预加载关联关系，减少 N+1 查询
  - 分页查询时限制 limit，避免一次性加载过多数据
- 连接与缓存
  - 启用 pool_pre_ping 提升连接稳定性
  - 对热点数据使用 Redis 缓存（如令牌、用户信息、权限集合），降低数据库压力
- 写入优化
  - 批量插入与合并操作，减少往返次数
  - 合理使用 flush/refresh，避免不必要的查询

## 故障排除指南
- 会话与事务
  - 若出现"未提交/未回滚"问题，检查依赖注入中是否正确捕获异常并执行回滚
  - 确保每个请求使用独立会话，避免跨请求共享会话
- 表结构不一致
  - 启动时调用 init_db() 创建表；若需迁移，请结合 Alembic 进行版本化管理
- 权限查询异常
  - 确认 role_permissions 关联表存在且外键约束正确
  - 检查用户权限查询是否使用 distinct 去重
- 缓存连接
  - Redis 客户端延迟初始化，注意并发场景下的连接复用与关闭

**章节来源**
- [src/infrastructure/database/connection.py:27-38](file://src/infrastructure/database/connection.py#L27-L38)
- [src/infrastructure/cache/redis_client.py:9-27](file://src/infrastructure/cache/redis_client.py#L9-L27)

## 结论
本数据模型以清晰的领域边界与分层架构为基础，通过 SQLAlchemy 2.0+ 异步 ORM 实现了用户、角色、权限与安全规则的完整建模。模型具备良好的扩展性与可维护性，配合仓储层与会话管理机制，能够满足高并发场景下的性能与一致性需求。建议后续引入版本化迁移工具以完善数据库演进管理。

## 附录
- 数据库迁移与版本管理建议
  - 使用 Alembic 进行迁移脚本管理，基于 SQLAlchemy 模型生成初始迁移
  - 将模型变更纳入版本控制，每次发布前评估迁移风险
  - 在生产环境执行迁移前进行备份与灰度验证
- 令牌与会话存储建议
  - 访问令牌短期有效，刷新令牌长期有效但需严格保管
  - 可将刷新令牌存储于安全的短时缓存中，结合黑名单机制实现撤销
- 审计与监控
  - 记录关键操作的时间戳与操作者信息，便于审计
  - 监控慢查询与高并发场景下的数据库指标，及时优化