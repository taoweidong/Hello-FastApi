...
- 更新了依赖关系分析以反映新的技术栈
- **新增**：角色-权限关联表重构为RolePermissionLink模型，使用link_model参数实现多对多关系
- **更新**：字段定义从mapped_column改为SQLModel的Field装饰器
- **更新**：多对多关系使用link_model参数替代传统的关联表
...
**更新**：新的实现引入了RolePermissionLink关联模型，使用SQLModel的link_model参数来管理角色和权限之间的多对多关系，提供了更清晰的关联表结构和更好的ORM支持。
...
- [rbac_repository.py:1-123](file://src/infrastructure/repositories/rbac_repository.py#L1-L123)
- [models.py:1-171](file://src/infrastructure/database/models.py#L1-L171)
- [connection.py:1-39](file://src/infrastructure/database/connection.py#L1-L39)

## 核心组件

RBAC系统的核心由四个实体构成，每个实体都有明确的职责和设计原则：
...
- **描述**：角色的详细说明
- **时间戳**：创建时间自动跟踪
- **权限关系**：通过RolePermissionLink模型实现多对多关系
- **用户关系**：维护用户与角色的关联
...
- **操作类型**：具体的权限动作
- **描述信息**：权限的详细说明
- **角色关系**：通过RolePermissionLink模型实现多对多关系
...
### 角色-权限关联实体（RolePermissionLink）

**新增**：RolePermissionLink是专门用于管理角色和权限之间多对多关系的关联模型：
- **复合主键**：role_id和permission_id组成的复合主键
- **外键约束**：双向外键确保数据完整性，支持级联删除
- **简化关系**：替代传统关联表的复杂实现

**章节来源**
- [models.py:17-28](file://src/infrastructure/database/models.py#L17-L28)
- [models.py:67-92](file://src/infrastructure/database/models.py#L67-L92)
- [models.py:95-119](file://src/infrastructure/database/models.py#L95-L119)
- [models.py:122-144](file://src/infrastructure/database/models.py#L122-L144)
...
- [rbac_repository.py:11-123](file://src/infrastructure/repositories/rbac_repository.py#L11-L123)
- [connection.py:27-39](file://src/infrastructure/database/connection.py#L27-L39)
...
Role "1" -- "*" Permission : 多对多(link_model=RolePermissionLink)
Role "1" -- "*" UserRole : 一对多
UserRole "1" -- "1" User : 多对一
Permission "1" -- "*" Role : 多对多(link_model=RolePermissionLink)
```
...
**章节来源**
- [models.py:72-78](file://src/infrastructure/database/models.py#L72-L78)
...
**章节来源**
- [models.py:100-109](file://src/infrastructure/database/models.py#L100-L109)
...
**图表来源**
- [models.py:122-144](file://src/infrastructure/database/models.py#L122-L144)
- [models.py:33-61](file://src/infrastructure/database/models.py#L33-L61)
...
**章节来源**
- [models.py:127-137](file://src/infrastructure/database/models.py#L127-L137)
...
RolePermissionLink "1" -- "1" Role : 多对一
RolePermissionLink "1" -- "1" Permission : 多对一
```
...
RolePermissionLink实体的关键设计特点：
- **复合主键**：role_id和permission_id组成的复合主键确保唯一性
- **外键约束**：双向外键确保数据完整性，支持级联删除
- **简化实现**：替代传统关联表的复杂实现，提供更清晰的ORM映射

#### 多对多关系管理

通过link_model参数实现多对多关系：
- **声明式关系**：在Role和Permission实体中使用link_model参数
- **自动管理**：SQLModel自动管理关联表的插入、更新和删除
- **查询优化**：支持高效的多对多查询和过滤
...
- [rbac_repository.py:114-122](file://src/infrastructure/repositories/rbac_repository.py#L114-L122)
...
**章节来源**
- [rbac_service.py:124-133](file://src/application/services/rbac_service.py#L124-L133)
- [rbac_repository.py:106-122](file://src/infrastructure/repositories/rbac_repository.py#L106-L122)
...
**图表来源**
- [models.py:33-144](file://src/infrastructure/database/models.py#L33-L144)
...
**章节来源**
- [models.py:38-48](file://src/infrastructure/database/models.py#L38-L48)
- [models.py:127-137](file://src/infrastructure/database/models.py#L127-L137)
...
- [connection.py:3-39](file://src/infrastructure/database/connection.py#L3-L39)
...
**问题症状**：角色和权限关系查询异常
**可能原因**：
- RolePermissionLink关联表数据不一致
- link_model参数配置错误
- 查询语句缺少必要的JOIN操作

**解决步骤**：
1. 检查RolePermissionLink表的数据完整性
2. 验证Role和Permission实体的link_model配置
3. 确认查询语句包含正确的JOIN操作
...
- **完善的错误处理**：提供友好的错误反馈机制
- **良好的扩展性**：支持未来功能扩展
- **现代化ORM支持**：使用SQLModel提供更好的开发体验

### 技术特色

- **UUID主键设计**：36字符长度的UUID支持分布式部署
- **多对多关系**：通过RolePermissionLink模型实现清晰的关联管理
- **link_model参数**：使用SQLModel的link_model参数简化多对多关系
- **级联操作**：确保数据完整性
...
**重要更新**：新的SQLAlchemy实现提供了更好的性能、异步支持和现代化的ORM特性，为未来的功能扩展奠定了坚实的基础。RolePermissionLink关联模型的引入进一步简化了多对多关系的管理，提供了更清晰的代码结构和更好的ORM支持。