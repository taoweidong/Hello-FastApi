...
## 项目结构
配置相关代码集中在 config 包内，核心为 settings.py 中的多环境配置类与工厂函数；应用通过 src/main.py 在启动阶段读取配置并注入到中间件、路由与异常处理等组件；日志与数据库等子系统也直接依赖配置对象；Docker Compose 将生产环境的关键配置以环境变量形式注入容器。

```mermaid
graph TB
subgraph "配置层"
S["settings.py<br/>定义 Settings 及其子类<br/>get_settings()/get_cached_settings()"]
PkgInit["config/__init__.py<br/>导出 settings 实例"]
end
subgraph "应用层"
...
ASGI["config/asgi.py<br/>生产 ASGI 入口"]
Manage["manage.py<br/>开发服务器运行依赖 settings"]
end
S --> PkgInit
PkgInit --> Main
S --> Logger
...
**图表来源**
- [config/settings.py:1-212](file://config/settings.py#L1-L212)
...
**章节来源**
- [config/settings.py:41-113](file://config/settings.py#L41-L113)
- [config/settings.py:115-156](file://config/settings.py#L115-L156)
- [config/settings.py:158-207](file://config/settings.py#L158-L207)
- [config/settings.py:210-212](file://config/settings.py#L210-L212)
...
**章节来源**
- [config/settings.py:89-97](file://config/settings.py#L89-L97)
- [config/settings.py:59-72](file://config/settings.py#L59-L72)
...
**章节来源**
- [config/settings.py:200-207](file://config/settings.py#L200-L207)
- [src/main.py:31-83](file://src/main.py#L31-L83)
- [src/core/logger.py:1-48](file://src/core/logger.py#L1-L48)
- [src/infrastructure/database/connection.py:1-51](file://src/infrastructure/database/connection.py#L1-L51)
...
**章节来源**
- [config/settings.py:55-72](file://config/settings.py#L55-L72)
...
- 开发环境
  ...
  - 通过 manage.py runserver 启动，reload 由 DEBUG 控制。
- 测试环境
...
- 生产环境
  ...
  - ASGI 入口由 config.asgi.py 导出，用于生产 WSGI/ASGI 服务器（如 uvicorn）托管。
  - Dockerfile 固化依赖安装与运行命令，日志目录挂载到宿主机持久化。

**章节来源**
- [config/settings.py:115-156](file://config/settings.py#L115-L156)
- [docker/docker-compose.yml:1-59](file://docker/docker-compose.yml#L1-59)
- [docker/Dockerfile:1-29](file://docker/Dockerfile#L1-L29)
- [config/asgi.py:1-6](file://config/asgi.py#L1-L6)
- [manage.py:14-21](file://manage.py#L14-L21)

## 依赖分析
- 模块耦合
  - config.settings 作为唯一配置源，被 src/main.py、src/core/logger.py、src/infrastructure/database/connection.py 广泛依赖。
  - config.__init__.py 仅负责导出 settings，降低上层模块的导入复杂度。
- 外部依赖
...
```mermaid
graph LR
Settings["config/settings.py"] --> Main["src/main.py"]
Settings --> Logger["src/core/logger.py"]
Settings --> DBConn["src/infrastructure/database/connection.py"]
Settings --> PkgInit["config/__init__.py"]
PkgInit --> Main
PkgInit --> Logger
PkgInit --> DBConn
DockerCompose["docker/docker-compose.yml"] --> ASGI["config/asgi.py"]
...
- 关键配置键说明
  ...
  - LOG_LEVEL：日志级别。
- 常见部署策略
  - 开发：本地 .env.development + manage.py runserver。
  - 测试：本地 .env.testing + 测试框架。
  - 生产：docker-compose.yml 注入环境变量 + config.asgi.py 托管。
