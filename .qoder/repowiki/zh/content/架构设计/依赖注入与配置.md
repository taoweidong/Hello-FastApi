...
- FastAPI 依赖注入机制的实现与配置：依赖函数的定义、作用域管理、依赖链解析与执行顺序。
...
- 领域与应用服务：认证与 RBAC 服务，依赖于仓储与数据库会话。
...
- 应用服务：认证服务与 RBAC 服务，分别依赖仓储与数据库会话完成业务逻辑。
...
## 架构总览
下图展示了依赖注入在请求生命周期中的流转：路由层通过依赖函数获取认证上下文与数据库会话，应用服务使用仓储与会话执行业务逻辑，最终返回响应。
...
## 依赖关系分析
下图展示依赖注入在各层之间的关系：路由层依赖认证与权限依赖，认证与权限依赖依赖数据库会话与令牌服务，应用服务依赖仓储与会话。
...
## 结论
本项目通过明确的依赖函数与依赖工厂，实现了认证、权限与数据库会话的解耦与可测试性。路由层仅负责声明依赖，业务逻辑集中在应用服务，配合仓储与领域服务，形成清晰的依赖注入链路。通过合理的依赖作用域管理与事务边界控制，提升了系统的可维护性与运行效率。
