...
- [__init__.py](file://service/src/infrastructure/common/__init__.py)
- [test_core.py](file://service/tests/unit/test_core.py)
</cite>

## 更新摘要
**所做更改**
- 新增基础设施通用工具模块文档，反映 src/core/utils.py 已移动到 src/infrastructure/common/utils.py
- 更新实用函数章节，包含 get_utc_now、is_valid_email、is_strong_password 函数说明
- 新增工具函数测试用例分析，展示密码强度和邮箱验证的测试覆盖
- 更新项目结构图，体现新的工具函数组织架构
- 新增工具函数导入导出机制说明
...
S_INFRA["src/infrastructure/common/utils.py<br/>基础设施通用工具<br/>UTC时间获取、邮箱验证、密码强度检查"]
end
subgraph "前端工程 web/"
...
**图表来源**
- [cli.py:1-285](file://service/scripts/cli.py#L1-L285)
...
- [plugins.ts:1-79](file://web/build/plugins.ts#L1-L79)
...
## 核心组件
本节聚焦三类核心工具体系：后端 CLI、基础设施通用工具和前端代码质量工具链。
...
- 后端 CLI 工具
  ...
  - seeddata：初始化完整的测试数据，包括菜单、日志等系统功能数据

- 基础设施通用工具
  - get_utc_now：获取当前 UTC 时间，用于系统日志和数据记录的时间戳
  - is_valid_email：验证邮箱格式的正则表达式检查，确保邮箱格式正确性
  - is_strong_password：检查密码强度要求，包括长度、大小写字母和数字验证
...
**章节来源**
- [cli.py:24-285](file://service/scripts/cli.py#L24-L285)
- [utils.py:7-27](file://service/src/infrastructure/common/utils.py#L7-L27)
- [eslint.config.js:1-191](file://web/eslint.config.js#L1-L191)
- [stylelint.config.js:1-88](file://web/stylelint.config.js#L1-L88)
- [.prettierrc.js:1-10](file://web/.prettierrc.js#L1-L10)
...
## 架构总览
下图展示了后端 CLI、基础设施通用工具与前端工具链之间的协作关系，以及开发环境初始化的整体流程。
...
- [cli.py:241-285](file://service/scripts/cli.py#L241-L285)
- [utils.py:1-27](file://service/src/infrastructure/common/utils.py#L1-L27)
...
- 组件职责
  ...
  - seeddata：生成完整的测试数据，包括菜单、登录日志、操作日志、系统日志等

- 关键流程
  - 命令解析与分发：根据传入参数选择对应处理器
  - 异步会话管理：使用异步上下文管理数据库会话，保证事务一致性
  - 数据去重策略：通过查询已存在记录避免重复创建
  - 测试数据生成：批量创建符合业务逻辑的测试数据

**更新** 新增完整的 CLI 命令行界面，提供 285 行功能强大的数据库管理和系统维护工具
...
**图表来源**
- [cli.py:24-31](file://service/scripts/cli.py#L24-L31)
- [cli.py:58-63](file://service/scripts/cli.py#L58-L63)
...
SeedCommit --> Done
SeedData --> MenuCheck["检查菜单数据"]
MenuCheck --> MenuAdd["添加系统菜单"]
MenuCheck --> LogCheck["检查登录日志"]
LogCheck --> LogAdd["添加登录日志"]
LogCheck --> OpLogCheck["检查操作日志"]
OpLogCheck --> OpLogAdd["添加操作日志"]
OpLogCheck --> SysLogCheck["检查系统日志"]
SysLogCheck --> SysLogAdd["添加系统日志"]
SysLogAdd --> SeedCommit
InitDB --> Done
Run --> Done
Done --> End(["结束"])
```
...
**图表来源**
- [cli.py:241-285](file://service/scripts/cli.py#L241-L285)
- [cli.py:34-56](file://service/scripts/cli.py#L34-L56)
- [cli.py:58-100](file://service/scripts/cli.py#L58-L100)
- [cli.py:102-238](file://service/scripts/cli.py#L102-L238)
...
### 基础设施通用工具分析
- 组件职责
  - get_utc_now：获取当前 UTC 时间，用于系统日志、数据记录和时间戳生成
  - is_valid_email：验证邮箱格式的正则表达式检查，确保邮箱格式正确性
  - is_strong_password：检查密码强度要求，包括长度、大小写字母和数字验证

- 工具函数特性
  - get_utc_now：返回带时区信息的 UTC 时间对象，确保全球时间一致性
  - is_valid_email：使用标准邮箱正则表达式，支持常见邮箱格式验证
  - is_strong_password：验证密码强度，要求至少8位字符，包含大写字母、小写字母和数字

- 导入导出机制
  - 通过 __init__.py 文件集中导出工具函数，提供统一的导入接口
  - 支持直接导入具体函数或批量导入所有工具函数

**更新** 新增基础设施通用工具模块，提供时间处理、邮箱验证、密码强度检查等实用函数

```mermaid
flowchart TD
Utils["infrastructure/common/utils.py"] --> Export["__init__.py 导出"]
Export --> Import1["from utils import get_utc_now"]
Export --> Import2["from utils import is_valid_email"]
Export --> Import3["from utils import is_strong_password"]
Export --> Import4["from utils import *"]
Import1 --> Usage1["获取UTC时间戳"]
Import2 --> Usage2["验证邮箱格式"]
Import3 --> Usage3["检查密码强度"]
Usage1 --> DB["数据库时间戳"]
Usage2 --> Auth["用户认证"]
Usage3 --> Security["安全验证"]
```

**图表来源**
- [utils.py:1-27](file://service/src/infrastructure/common/utils.py#L1-L27)
- [__init__.py:1-6](file://service/src/infrastructure/common/__init__.py#L1-L6)
...
### 工具函数测试用例分析
- 测试覆盖范围
  - 邮箱验证测试：验证有效邮箱格式（如 test@example.com、user.name@domain.org）和无效邮箱格式（如 invalid、@domain.com、user@）
  - 密码强度测试：验证强密码（如 Test1234、MyP@ssw0rd）和弱密码的各种情况（短密码、缺少大写字母、缺少小写字母、缺少数字、缺少数字）

- 测试策略
  - 使用 pytest 框架进行单元测试
  - 分别测试邮箱验证和密码强度两个独立的功能模块
  - 覆盖边界条件和典型使用场景
...
Test->>Utils : 导入 is_valid_email 和 is_strong_password
Test->>Utils : 验证有效邮箱格式
Utils-->>Test : 返回 True
Test->>Utils : 验证无效邮箱格式
Utils-->>Test : 返回 False
...
**更新** 开发环境初始化流程现已集成新的 CLI 命令和基础设施通用工具，提供更完整的环境搭建体验
...
**图表来源**
- [vite.config.ts:12-73](file://web/vite.config.ts#L12-L73)
- [plugins.ts:17-79](file://web/build/plugins.ts#L17-L79)
...
- 后端
  ...
  - CLI 命令采用异步设计，支持并发处理多个数据库操作
  - 基础设施通用工具函数使用高效的正则表达式和内置函数
...
**更新** 新增基础设施通用工具模块的性能优化考虑，包括高效正则表达式和内置函数的使用

**章节来源**
- [cli.py:76-100](file://service/scripts/cli.py#L76-L100)
- [utils.py:12-27](file://service/src/infrastructure/common/utils.py#L12-L27)
...
- 后端
  ...
  - CLI 命令执行错误：检查命令参数格式；确认依赖包已正确安装
  - 工具函数导入失败：检查模块路径是否正确；确认 __init__.py 导出配置
...
- 基础设施通用工具模块提供时间处理、邮箱验证、密码强度检查等实用函数
- 前端 ESLint/Stylelint/Prettier 与 Vite 插件生态保障了代码质量与开发体验
- 通过 API 验证脚本与开发脚本的协同，形成从环境到功能的闭环验证

**更新** 新增的基础设施通用工具模块提供了 27 行功能强大的实用函数，包括时间处理、邮箱验证、密码强度检查等，进一步提升了开发效率和系统可维护性

建议团队在日常工作中：
- 使用开发脚本进行一键初始化，确保环境一致性
- 在提交前执行 lint 与格式化脚本，减少代码审查负担
- 结合 Vite 插件与构建配置，持续优化首屏性能与包体大小
- 利用 CLI 命令行界面进行数据库管理和系统维护，提高工作效率
- 使用基础设施通用工具函数进行数据验证和时间处理，确保数据一致性

## 附录
- 实际使用示例（路径指引）
  - 启动开发服务器：参考 [cli.py:24-31](file://service/scripts/cli.py#L24-L31)
  - 创建超级管理员：参考 [cli.py:34-56](file://service/scripts/cli.py#L34-L56)
  - 初始化数据库：参考 [cli.py:58-63](file://service/scripts/cli.py#L58-L63)
  - 初始化 RBAC 数据：参考 [cli.py:66-99](file://service/scripts/cli.py#L66-L99)
  - 初始化测试数据：参考 [cli.py:102-238](file://service/scripts/cli.py#L102-L238)
  - 获取 UTC 时间：参考 [utils.py:7-9](file://service/src/infrastructure/common/utils.py#L7-L9)
  - 验证邮箱格式：参考 [utils.py:12-15](file://service/src/infrastructure/common/utils.py#L12-L15)
  - 检查密码强度：参考 [utils.py:18-26](file://service/src/infrastructure/common/utils.py#L18-L26)
  ...
  - Vite 构建配置：参考 [vite.config.ts:12-73](file://web/vite.config.ts#L12-L73)
  - 插件清单：参考 [plugins.ts:17-79](file://web/build/plugins.ts#L17-L79)
