...
- [.gitignore](file://service/.gitignore)
...
## 更新摘要
**所做更改**
- 更新了环境配置管理部分，反映从多环境文件简化为单一.env文件的变更
- 更新了配置文件组织结构与优先级规则
- 更新了.gitignore文件管理说明
- 更新了环境切换与配置加载顺序的说明
...
- [pyproject.toml:1-78](file://service/pyproject.toml#L1-L78)
- [README.md:159-197](file://service/README.md#L159-L197)
- [.env.example:1-63](file://service/.env.example#L1-L63)
- [.gitignore:1-51](file://service/.gitignore#L1-L51)
...
### .gitignore 文件管理
- Git 忽略规则：
  - Python 缓存文件：__pycache__、*.pyc、*.pyo、*.pyd、$py.class
  - 虚拟环境：.venv、venv、env
  - IDE 配置：.idea、.vscode、*.swp、*.swo
  - 环境配置：.env
  - 数据库文件：sql/*.db、sql/*.sqlite3、*.db-journal
  - 日志文件：logs/*.log
  - Docker 配置：docker/.env
  - 代码覆盖率：htmlcov、.coverage、.coverage.*、coverage.xml
  - MyPy 缓存：.mypy_cache/
  - Ruff 缓存：.ruff_cache/
...
- .env 文件未被识别：
  - 确认 .env 文件位于项目根目录且具有正确的权限。
  - 检查 .gitignore 是否正确配置以忽略 .env 文件。

**章节来源**
- [settings.py:84-92](file://service/src/config/settings.py#L84-L92)
- [settings.py:144-198](file://service/src/config/settings.py#L144-L198)
- [cli.py:24-31](file://service/scripts/cli.py#L24-L31)
- [.gitignore:22-24](file://service/.gitignore#L22-L24)
...
- .env.example 提供了完整的环境变量配置模板和使用说明。
