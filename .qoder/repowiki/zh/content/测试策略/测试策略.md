...
## 更新摘要
**所做更改**
- 更新测试目录结构：从 src/tests/ 迁移到 tests/ 根目录结构
- 更新 pytest 配置：testpaths 从 ['src/tests'] 改为 ['tests']
- 更新所有测试文件引用路径以反映新的目录结构
- 更新测试架构图和示例路径
...
## 项目结构
项目采用按"层次+特性"混合组织方式，测试目录已迁移至项目根目录的 tests 下，分为 unit、integration 子目录，配合 pytest 配置集中于 pyproject.toml。

**更新** 测试目录结构已从 src/tests/ 迁移到 tests/ 根目录，pytest 配置中的 testpaths 已相应更新
...
- [pyproject.toml:70-77](file://pyproject.toml#L70-L77)
- [conftest.py:14-58](file://tests/conftest.py#L14-L58)
...
- [conftest.py:46-57](file://tests/conftest.py#L46-L57)
...
- pytest 配置
  - 测试路径、异步模式、自定义标记
  - **更新** testpaths 已从 ['src/tests'] 改为 ['tests']
- 代码质量
  - Ruff：格式检查与规则检查
  - MyPy：类型检查
- 并行执行
...
- 相关文件路径
  - [依赖注入与客户端固件:46-57](file://tests/conftest.py#L46-L57)
  - [数据库生命周期固件:29-36](file://tests/conftest.py#L29-L36)
...
## 结论
本项目已具备清晰的测试金字塔基础：单元测试覆盖核心工具与领域服务，集成测试覆盖 API 与数据库交互，端到端测试通过脚本验证完整业务流。测试目录结构已优化为更简洁的 tests/ 根目录布局，pytest 配置已相应更新。建议在现有基础上补充：
