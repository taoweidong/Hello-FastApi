...
- [test_validators.py](file://service/tests/unit/test_validators.py)
</cite>

## 更新摘要
**所做更改**
- 验证器系统重构：src/core/validators.py 已迁移至 src/application/validators.py
- 新增基于Pydantic field_validator装饰器的集中式验证系统
- 实现统一的ID规范化工具（normalize_optional_id）
- 增强DTO验证机制，包括空字符串处理、数值转换和ID标准化
- 新增部门管理相关DTO类及其验证机制
...
**更新** 本次更新重点反映了数据验证系统的重大改进，验证器系统已从核心层迁移至应用层，新增了基于Pydantic field_validator装饰器的集中式验证系统，实现了统一的ID规范化工具和增强的DTO验证机制，包括用户名和密码验证器等核心功能。
...
DTO6["角色DTO<br/>role_dto.py"]
DTO7["权限DTO<br/>permission_dto.py"]
end
R1 --> S1
R2 --> S2
R3 --> S3
...
- **集中式验证系统**：基于Pydantic的field_validator装饰器，实现统一的字段级验证机制，包括空字符串处理、数值转换和ID标准化。
- **ID规范化工具**：提供统一的ID处理函数，支持空字符串、零值和None的标准化处理，确保数据一致性。
- **自定义验证器**：在应用层提供通用格式与强度校验（如邮箱、密码强度、用户名）。
- **业务规则校验**：在应用服务层结合仓储与领域服务执行业务约束（如唯一性、状态、权限）。
- **异常体系**：统一的验证错误与业务错误类型，便于上层捕获与本地化处理。
- **密码服务**：提供哈希与校验，保障密码安全存储与比对。

**更新** 验证器系统已从核心层迁移至应用层，新增的集中式验证系统通过field_validator装饰器实现了统一的数据验证机制，显著提升了验证的一致性和可维护性。
...
participant Av as "应用层验证器(validators.py)"
...
**更新** 新增的DTO类引入了基于field_validator装饰器的集中式验证机制，实现了统一的数据清理和标准化处理。
...
### 集中式验证系统与field_validator装饰器
- **统一验证机制**：通过Pydantic的field_validator装饰器实现集中式的字段级验证，替代传统的函数式验证。
- **ID规范化**：提供normalize_optional_id函数，统一处理可选ID字段（空字符串、0、'0'、None）。
- **空字符串处理**：empty_str_to_none和empty_str_or_zero_to_none函数，智能处理前端传递的空值。
- **数值转换**：自动将字符串数值转换为整型，支持None和空字符串的统一处理。

```mermaid
flowchart TD
Start(["进入DTO验证"]) --> FieldValidator["field_validator装饰器"]
FieldValidator --> EmptyStrCheck["空字符串检查"]
EmptyStrCheck --> NormalizeID["ID规范化处理"]
NormalizeID --> NumericConvert["数值转换"]
NumericConvert --> StatusParse["状态解析"]
StatusParse --> Ok["验证通过"]
```
...
### ID规范化工具与数据清理
- **normalize_optional_id**：统一处理可选ID字段，将空字符串、0、'0'、None转换为None，整型转换为字符串。
- **empty_str_to_none**：将空字符串转换为None，支持前端传递的空值处理。
- **empty_str_or_zero_to_none**：将空字符串或0转换为None，常用于可选整型字段。
- **parse_time_range**：解析时间范围参数，支持数组和字符串格式。
- **parse_status**：解析状态参数，将字符串状态转换为整型。

**更新** 新增的ID规范化工具显著提升了数据处理的一致性和准确性，减少了重复的验证逻辑。
...
### 自定义验证器与格式校验
- 应用层提供通用格式与强度校验函数，便于在服务层或业务逻辑中复用。
- 关键能力：
  - 邮箱格式校验
  - 密码强度校验（长度、大小写字母、数字）
  - 用户名格式校验（长度与字符集）
...
**更新** 新增的集中式验证系统通过field_validator装饰器实现了统一的数据清理机制，显著提升了数据处理的效率和一致性。
...
- **开发步骤**：
  ...
  - 在服务层调用并结合异常体系抛出统一错误。
- **示例参考**：
  - 邮箱/密码强度/用户名格式的实现路径见应用层验证器与工具函数。
...
- **新增优化点**：
  ...
  - field_validator装饰器的性能优化，避免不必要的验证调用。

## 依赖分析
- API 层依赖应用服务与 DTO；应用服务依赖仓储与领域服务；应用层验证器提供验证与工具；异常体系贯穿各层。
- 依赖方向清晰，耦合度低，便于扩展与替换。
- **新增依赖关系**：
  - 系统路由依赖部门服务和日志服务。
  - 部门服务依赖部门仓储。
...
## 结论
该验证体系以 Pydantic DTO 为基础，结合应用层通用验证器与应用层业务规则，形成从输入到输出的全链路校验闭环。通过统一异常与可测试的验证器，既保证了数据质量，也为扩展与维护提供了清晰路径。本次更新大幅扩展了验证覆盖范围，验证器系统已从核心层迁移至应用层，新增了基于field_validator装饰器的集中式验证系统、统一的ID规范化工具和增强的DTO验证机制，进一步提升了系统的完整性和可靠性。建议在现有基础上完善本地化与第三方库集成，持续提升可用性与可维护性。

## 附录

### 常用验证器清单与调用位置
- 邮箱格式：核心工具函数，适合在服务层或业务逻辑中调用。
- 密码强度：应用层验证器，适合在注册/修改密码流程中调用。
- 用户名格式：应用层验证器，适合在注册/登录流程中调用。
...
### 集中式验证系统实现细节
- **field_validator装饰器**：在DTO类中使用，实现字段级验证逻辑。
- **ID规范化**：通过normalize_optional_id统一处理ID字段，确保数据一致性。
- **空字符串处理**：通过empty_str_to_none和empty_str_or_zero_to_none处理前端传递的空值。
- **数值转换**：自动将字符串数值转换为整型，支持None和空字符串的统一处理。
