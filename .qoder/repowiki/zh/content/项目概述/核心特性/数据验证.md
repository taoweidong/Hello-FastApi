...
## 核心组件
- Pydantic DTO：在应用层定义请求/响应的数据结构与约束，如最小长度、最大长度、可空性、别名等。
...
## 架构总览
下图展示从 API 请求到服务处理再到验证与异常的整体流程。

```mermaid
sequenceDiagram
participant Client as "客户端"
participant Router as "路由(auth_routes/user_routes)"
participant Service as "应用服务(auth_service/user_service)"
...
Client->>Router : "POST /api/system/(auth|user)"
...
- 示例要点：
  ...
  - 可空性：Union 类型或默认值
  - 别名：alias 用于兼容前端字段命名
  - 属性映射：model_config 控制 from_attributes/populate_by_name
...
## 结论
该验证体系以 Pydantic DTO 为基础，结合核心层通用验证器与应用层业务规则，形成从输入到输出的全链路校验闭环。通过统一异常与可测试的验证器，既保证了数据质量，也为扩展与维护提供了清晰路径。建议在现有基础上完善本地化与第三方库集成，持续提升可用性与可维护性。
