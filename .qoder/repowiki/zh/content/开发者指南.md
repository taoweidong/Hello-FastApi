...
- [src/core/decorators.py](file://src/core/decorators.py)
...
CORE5["src/core/utils.py"]
CORE6["src/core/validators.py"]
CORE7["src/core/decorators.py"]
end
subgraph "应用层"
APP1["src/application/services/*"]
APP2["src/application/dto/*"]
end
subgraph "领域层"
...
- [src/core/validators.py:1-26](file://src/core/validators.py#L1-L26)
- [src/core/decorators.py:1-23](file://src/core/decorators.py#L1-L23)
...
- 工具函数与验证器：提供UTC时间获取、邮箱格式验证、密码强度检查等通用工具函数。
- 防御性编程装饰器：提供函数执行时间记录装饰器，增强代码可观察性。
...
**更新** 命令行接口已从 `python manage.py <command>` 迁移到 `python -m scripts.cli <command>`，提供更清晰的模块化命令行工具。
...
**更新** 项目现已全面实施类型注解与防御性编程指导原则，确保代码质量和运行时安全性。

#### 类型注解标准
- **函数参数与返回值**：所有函数必须提供完整的类型注解，包括AsyncSession、User、TokenResponseDTO等复杂类型。
- **可选类型**：使用Union类型（如str | None）明确表示可空值，避免隐式None处理。
- **泛型类型**：列表类型使用list[Type]语法，如list[User]、list[str]。
- **SQLModel字段**：数据库模型字段使用Field注解，支持长度限制、唯一性约束等元数据。

#### 防御性编程实践
- **输入验证**：所有外部输入必须经过验证，使用Pydantic EmailStr、SQLModel Field约束等。
- **边界检查**：对数组访问、字符串截取等操作进行边界检查，防止IndexError。
- **异常处理**：使用专门的异常类（ValidationError、UnauthorizedError等）替代裸露的Exception。
- **资源清理**：确保数据库连接、文件句柄等资源在异常情况下也能正确释放。

#### 代码质量改进
- **装饰器模式**：使用@log_execution装饰器记录函数执行时间，增强代码可观察性。
- **工具函数**：提供is_valid_email、is_strong_password等验证函数，统一验证逻辑。
- **DTO设计**：使用SQLModel定义数据传输对象，确保前后端数据结构一致性。
- **仓储接口**：定义抽象仓储接口，支持依赖注入与测试替身。
...
**章节来源**
- [pyproject.toml:1-76](file://pyproject.toml#L1-L76)
...
**更新** 命令行工具已从 `python manage.py` 迁移到 `python -m scripts.cli`，请使用新的命令格式。
...
## 结论
本指南提供了从代码规范、开发流程、工具配置到性能优化与故障排除的完整实践路径。建议团队在日常开发中遵循统一的命名与注释规范，严格执行代码质量检查与测试策略，持续优化数据库与缓存方案，并通过中间件与日志体系提升可观测性与安全性。新的CLI管理工具提供了更灵活的开发工作流程，替代了传统的Django管理命令系统。项目现已全面实施类型注解与防御性编程指导原则，确保代码质量和运行时安全性。
...
**更新** 命令行接口已从 `python manage.py` 迁移到 `python -m scripts.cli`，请使用新的命令格式。
...
**更新** CLI工具已从 `manage.py` 命令迁移到模块化命令行接口，支持 `python -m scripts.cli <command>` 格式。
...
**更新** 项目已从传统的 `python manage.py <command>` 命令行接口迁移到新的 `python -m scripts.cli <command>` 模块化接口。
...
#### CLI工具特性
- **模块化设计**：通过 `python -m scripts.cli` 直接调用模块
- **异步支持**：所有数据库操作使用异步I/O
- **自动路径处理**：自动添加项目根目录到Python路径
- **统一错误处理**：提供清晰的错误信息与帮助文本

#### 开发脚本更新
开发环境初始化脚本中的命令已相应更新：
- `python manage.py initdb` → `python -m scripts.cli initdb`
- `python manage.py seedrbac` → `python -m scripts.cli seedrbac`
- `python manage.py runserver` → `python -m scripts.cli runserver`

#### 可执行命令
项目还提供了可执行命令入口：
```bash
hello-fastapi runserver       # 启动开发服务器
hello-fastapi createsuperuser # 创建超级用户
hello-fastapi initdb          # 初始化数据库
hello-fastapi seedrbac        # 填充 RBAC 初始数据
```

**章节来源**
- [scripts/cli.py:1-135](file://scripts/cli.py#L1-L135)
- [scripts/setup_dev.sh:30-46](file://scripts/setup_dev.sh#L30-L46)
- [scripts/setup_dev.bat:30-43](file://scripts/setup_dev.bat#L30-L43)
- [pyproject.toml:41-42](file://pyproject.toml#L41-L42)
- [README.md:181-188](file://README.md#L181-L188)
...
#### 类型注解实施标准
- **函数签名**：所有函数参数与返回值必须提供类型注解
- **变量声明**：复杂数据结构使用明确的类型注解
- **可选类型**：使用Union语法表示可空值
- **泛型类型**：使用现代Python泛型语法
- **SQLModel集成**：数据库模型字段使用Field注解

#### 防御性编程实践清单
- **输入验证**：使用Pydantic EmailStr、SQLModel Field约束
- **边界检查**：对数组访问、字符串操作进行边界验证
- **异常处理**：使用专用异常类替代裸露Exception
- **资源管理**：确保异常情况下的资源正确释放
- **日志记录**：使用@log_execution装饰器记录执行时间

#### 代码质量检查工具
- **MyPy类型检查**：在CI中强制执行类型检查
- **Ruff格式化**：统一代码风格与格式
- **pytest测试**：确保代码功能正确性
- **覆盖率报告**：监控测试覆盖率
...
- [scripts/lint.sh:14-15](file://scripts/lint.sh#L14-L15)