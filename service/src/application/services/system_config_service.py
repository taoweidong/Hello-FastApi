...
class SystemConfigService:
    ...
    async def create_config(self, dto: SystemConfigCreateDTO) -> SystemConfigResponseDTO:
        ...
        config_entity = SystemConfigEntity.create_new(
            key=dto.key,
            value=dto.value,
    ...
    async def get_configs(self, query: SystemConfigListQueryDTO) -> tuple[list[SystemConfigResponseDTO], int]:
        ...
        configs = await self.config_repo.get_all(page_num=query.pageNum, page_size=query.pageSize, key=query.key, is_active=query.isActive)
