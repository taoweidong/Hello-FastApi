...
class IPRuleService:
    ...
    def __init__(self, session: AsyncSession, ip_rule_repo: IPRuleRepositoryInterface):
    ...
    async def get_ip_rule(self, rule_id: str) -> IPRule:
        """获取单个 IP 规则。"""
        rule = await self.ip_rule_repo.get_ip_rule_by_id(session=self.session, rule_id=rule_id)
        if rule is None:
    ...
    async def update_ip_rule(self, rule_id: str, ip_address: str | None = None, rule_type: str | None = None, reason: str | None = None, is_active: int | None = None, expires_at: datetime | None = None, description: str | None = None) -> IPRule:
        """更新 IP 规则。"""
        rule = await self.ip_rule_repo.get_ip_rule_by_id(session=self.session, rule_id=rule_id)
        if rule is None:
            raise NotFoundError("IP规则不存在")
        ...
        result = await self.ip_rule_repo.update_ip_rule(session=self.session, rule=rule)
        await self.session.flush()
        return result

    async def delete_ip_rules(self, rule_ids: list[str]) -> int:
        """批量删除 IP 规则。"""
        count = await self.ip_rule_repo.delete_ip_rules(session=self.session, rule_ids=rule_ids)
        await self.session.flush()
        return count

    async def clear_ip_rules(self) -> int:
        """清空所有 IP 规则。"""
        count = await self.ip_rule_repo.clear_ip_rules(session=self.session)
        await self.session.flush()
        return count
