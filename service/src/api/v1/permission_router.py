...
class PermissionRouter(Routable):
    ...
    async def get_permission_list(
        self, pageNum: int = Query(1, ge=1, description="页码"), pageSize: int = Query(10, ge=1, le=100, description="每页条数"), permissionName: str = Query(None, description="权限名称"), service: PermissionService = Depends(get_permission_service), _: dict = Depends(require_permission("permission:view"))
    ) -> dict:
        """获取权限列表接口（分页）。"""
    ...
    @post("/")
    async def create_permission(self, dto: PermissionCreateDTO, service: PermissionService = Depends(get_permission_service), _: dict = Depends(require_permission("permission:manage"))) -> dict:
        """创建权限接口。"""
    ...
    async def delete_permission(self, permission_id: str, service: PermissionService = Depends(get_permission_service), _: dict = Depends(require_permission("permission:manage"))) -> dict:
        """删除权限接口。"""
