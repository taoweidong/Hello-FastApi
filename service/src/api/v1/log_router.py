...
class LogRouter(Routable):
    ...
    @post("/login-logs")
    async def get_login_logs(self, query: LoginLogListQueryDTO = Body(default={}), service: LogService = Depends(get_log_service), current_user: dict = Depends(get_current_active_user)) -> dict:
        """获取登录日志列表。"""
    ...
    async def batch_delete_login_logs(self, dto: BatchDeleteLogDTO, service: LogService = Depends(get_log_service), current_user: dict = Depends(get_current_active_user)) -> dict:
        """批量删除登录日志。"""
    ...
    async def clear_login_logs(self, service: LogService = Depends(get_log_service), current_user: dict = Depends(get_current_active_user)) -> dict:
        """清空所有登录日志。"""
        count = await service.clear_login_logs()
        return success_response(data={"deleted": count}, message=f"已清空 {count} 条记录")

    @post("/operation-logs")
    async def get_operation_logs(self, query: OperationLogListQueryDTO = Body(default={}), service: LogService = Depends(get_log_service), current_user: dict = Depends(get_current_active_user)) -> dict:
        """获取操作日志列表。"""
    ...
    async def batch_delete_operation_logs(self, dto: BatchDeleteLogDTO, service: LogService = Depends(get_log_service), current_user: dict = Depends(get_current_active_user)) -> dict:
        """批量删除操作日志。"""
        count = await service.delete_operation_logs(dto)
        return success_response(data={"deleted": count}, message=f"已删除 {count} 条记录")

    @post("/operation-logs/clear")
    async def clear_operation_logs(self, service: LogService = Depends(get_log_service), current_user: dict = Depends(get_current_active_user)) -> dict:
        """清空所有操作日志。"""
        count = await service.clear_operation_logs()
        return success_response(data={"deleted": count}, message=f"已清空 {count} 条记录")

    @post("/system-logs")
    async def get_system_logs(self, query: SystemLogListQueryDTO = Body(default={}), service: LogService = Depends(get_log_service), current_user: dict = Depends(get_current_active_user)) -> dict:
        """获取系统日志列表。"""
    ...
    async def get_system_logs_detail(self, data: dict = Body(default={}), service: LogService = Depends(get_log_service), current_user: dict = Depends(get_current_active_user)) -> dict:
        """获取系统日志详情。"""
        log_id = data.get("id")
        if not log_id:
    ...
    async def batch_delete_system_logs(self, dto: BatchDeleteLogDTO, service: LogService = Depends(get_log_service), current_user: dict = Depends(get_current_active_user)) -> dict:
        """批量删除系统日志。"""
        count = await service.delete_system_logs(dto)
        return success_response(data={"deleted": count}, message=f"已删除 {count} 条记录")

    @post("/system-logs/clear")
    async def clear_system_logs(self, service: LogService = Depends(get_log_service), current_user: dict = Depends(get_current_active_user)) -> dict:
        """清空所有系统日志。"""
