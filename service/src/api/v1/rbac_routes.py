...
from src.api.dependencies import get_rbac_service, get_role_repository, require_permission
...
@role_router.post("")
async def get_role_list(query: RoleListQueryDTO, service: RBACService = Depends(get_rbac_service), _: dict = Depends(require_permission("role:view"))):
    """获取角色列表接口（分页）。
...
async def create_role(dto: RoleCreateDTO, service: RBACService = Depends(get_rbac_service), _: dict = Depends(require_permission("role:manage"))):
    """创建角色接口。
...
async def get_role(role_id: str, service: RBACService = Depends(get_rbac_service), _: dict = Depends(require_permission("role:view"))):
    """获取角色详情接口。
...
async def update_role(role_id: str, dto: RoleUpdateDTO, service: RBACService = Depends(get_rbac_service), _: dict = Depends(require_permission("role:manage"))):
    """更新角色接口。
...
async def delete_role(role_id: str, service: RBACService = Depends(get_rbac_service), _: dict = Depends(require_permission("role:manage"))):
    """删除角色接口。
...
async def assign_permissions(role_id: str, dto: AssignPermissionsDTO, service: RBACService = Depends(get_rbac_service), _: dict = Depends(require_permission("role:manage"))):
    """为角色分配权限接口。
...
async def get_permission_list(pageNum: int = Query(1, ge=1, description="页码"), pageSize: int = Query(10, ge=1, le=100, description="每页条数"), permissionName: str = Query(None, description="权限名称"), service: RBACService = Depends(get_rbac_service), _: dict = Depends(require_permission("permission:view"))):
    """获取权限列表接口（分页）。
...
async def create_permission(dto: PermissionCreateDTO, service: RBACService = Depends(get_rbac_service), _: dict = Depends(require_permission("permission:manage"))):
    """创建权限接口。
...
async def delete_permission(permission_id: str, service: RBACService = Depends(get_rbac_service), _: dict = Depends(require_permission("permission:manage"))):
    """删除权限接口。
