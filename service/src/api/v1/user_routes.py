...
@router.post("")
async def get_user_list(query: UserListQueryDTO, service: UserService = Depends(get_user_service), _: dict = Depends(require_permission("user:view"))):
    """获取用户列表接口（支持筛选和分页）。
...
async def create_user(dto: UserCreateDTO, service: UserService = Depends(get_user_service), _: dict = Depends(require_permission("user:add"))):
    """创建用户接口。
...
@router.get("/info")
async def get_current_user_info(user_id: str = Depends(get_current_user_id), service: UserService = Depends(get_user_service)):
    """获取当前登录用户信息接口。
...
@router.get("/{user_id}")
async def get_user_detail(user_id: str, service: UserService = Depends(get_user_service), _: dict = Depends(require_permission("user:view"))):
    """获取用户详情接口。
...
@router.put("/{user_id}")
async def update_user(user_id: str, dto: UserUpdateDTO, service: UserService = Depends(get_user_service), _: dict = Depends(require_permission("user:edit"))):
    """更新用户接口。
...
async def delete_user(user_id: str, service: UserService = Depends(get_user_service), _: dict = Depends(require_permission("user:delete"))):
    """删除用户接口。
...
async def batch_delete_users(dto: BatchDeleteDTO, service: UserService = Depends(get_user_service), _: dict = Depends(require_permission("user:delete"))):
    """批量删除用户接口。
...
async def reset_user_password(user_id: str, dto: ResetPasswordDTO, service: UserService = Depends(get_user_service), _: dict = Depends(require_permission("user:edit"))):
    """重置用户密码接口（管理员功能）。
...
async def update_user_status(user_id: str, dto: UpdateStatusDTO, service: UserService = Depends(get_user_service), _: dict = Depends(require_permission("user:edit"))):
    """更改用户状态接口。
...
async def change_password(dto: ChangePasswordDTO, user_id: str = Depends(get_current_user_id), service: UserService = Depends(get_user_service)):
    """修改当前用户密码接口。
...
async def assign_role(dto: AssignRoleDTO, service: UserService = Depends(get_user_service), _: dict = Depends(require_permission("user:edit"))):
    """为用户分配角色接口。
