...
async def get_online_logs(data: dict = Body(default={}), current_user: dict = Depends(get_current_active_user)):
    """获取在线用户列表（stub 数据）。
...
async def get_map_info(current_user: dict = Depends(get_current_active_user)):
    """获取地图数据（stub 数据）。
...
async def get_card_list(data: dict = Body(default={}), current_user: dict = Depends(get_current_active_user)):
    """获取卡片列表（stub 数据）。
