...
class SystemConfigRouter(Routable):
    ...
    async def get_config_list(
        ...
        _: dict = Depends(require_permission("config:view")),
    ) -> dict:
        """获取系统配置列表（分页）。"""
        configs, total = await service.get_configs(query)
        return list_response(
            list_data=[c.model_dump() for c in configs],
            total=total,
    ...
    async def create_config(
        ...
        service: SystemConfigService = Depends(get_system_config_service),
        _: dict = Depends(require_permission("config:add")),
    ) -> dict:
        """创建系统配置。"""
    ...
    async def update_config(
        ...
        service: SystemConfigService = Depends(get_system_config_service),
        _: dict = Depends(require_permission("config:edit")),
    ) -> dict:
        """更新系统配置。"""
    ...
    async def delete_config(
        ...
        config_id: str,
        service: SystemConfigService = Depends(get_system_config_service),
        _: dict = Depends(require_permission("config:delete")),
    ) -> dict:
        """删除系统配置。"""
