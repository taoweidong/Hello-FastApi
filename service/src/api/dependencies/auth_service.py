...
async def get_auth_service(db: AsyncSession = Depends(get_db), token_service: TokenService = Depends(get_token_service), password_service: PasswordService = Depends(get_password_service), cache_service: CacheService = Depends(get_cache_service)) -> AuthService:
    """获取认证服务实例。
