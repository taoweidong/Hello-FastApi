...
async def get_user_service(db: AsyncSession = Depends(get_db), password_service: PasswordService = Depends(get_password_service), cache_service: CacheService = Depends(get_cache_service)) -> UserService:
    """获取用户服务实例。
