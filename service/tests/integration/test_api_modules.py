...
async def _superuser_bearer(session: AsyncSession, username: str) -> str:
    ...
    user = await service.create_superuser(
        UserCreateDTO(username=username, email=f"{username}@example.com", password="TestPass123", nickname=username)
    )
...
class TestRoleEndpoints:
    async def test_get_role_list(self, client: AsyncClient, db_session: AsyncSession):
        ...
        response = await client.post(
            "/api/system/role", headers={"Authorization": auth}, json={"pageNum": 1, "pageSize": 10}
        )
    ...
    async def test_create_role(self, client: AsyncClient, db_session: AsyncSession):
        ...
        response = await client.post(
            ...
            json={"name": "test_role", "code": "test_role", "description": "测试角色", "isActive": 1},
        )
        assert response.status_code == 200
        result = response.json()
    ...
    async def test_get_role_detail(self, client: AsyncClient, db_session: AsyncSession):
        auth = await _superuser_bearer(db_session, "su_role_detail")
        response = await client.post(
            "/api/system/role/create",
            headers={"Authorization": auth},
            json={"name": "detail_role", "code": "detail_role", "description": "详情测试角色", "isActive": 1},
        )
        role_id = response.json()["data"]["id"]
        await db_session.commit()

        response = await client.get(f"/api/system/role/{role_id}", headers={"Authorization": auth})
        assert response.status_code == 200
        result = response.json()
        assert result["code"] == 0
    ...
    async def test_update_role(self, client: AsyncClient, db_session: AsyncSession):
        ...
        response = await client.post(
            "/api/system/role/create",
            headers={"Authorization": auth},
            json={"name": "update_role", "code": "update_role", "description": "原描述", "isActive": 1},
        )
        role_id = response.json()["data"]["id"]
        await db_session.commit()

        response = await client.put(
            f"/api/system/role/{role_id}", headers={"Authorization": auth}, json={"description": "更新后的描述"}
        )
        assert response.status_code == 200
        result = response.json()
        assert result["code"] == 0
    ...
    async def test_delete_role(self, client: AsyncClient, db_session: AsyncSession):
        ...
        response = await client.post(
            "/api/system/role/create",
            headers={"Authorization": auth},
            json={"name": "delete_role", "code": "delete_role", "description": "待删除角色", "isActive": 1},
        )
        role_id = response.json()["data"]["id"]
        await db_session.commit()

        response = await client.delete(f"/api/system/role/{role_id}", headers={"Authorization": auth})
        assert response.status_code == 200
...
class TestMenuEndpoints:
    async def test_get_menu_list(self, client: AsyncClient, db_session: AsyncSession):
        ...
        response = await client.post("/api/system/menu", headers={"Authorization": auth})
        assert response.status_code == 200
        result = response.json()
        assert result["code"] == 0
    ...
    async def test_get_menu_tree(self, client: AsyncClient, db_session: AsyncSession):
        ...
        response = await client.get("/api/system/menu/tree", headers={"Authorization": auth})
        assert response.status_code == 200
        result = response.json()
        assert result["code"] == 0

    async def test_get_user_menus(self, client: AsyncClient, db_session: AsyncSession):
        auth = await _superuser_bearer(db_session, "su_user_menus")
        response = await client.get("/api/system/menu/user-menus", headers={"Authorization": auth})
        assert response.status_code == 200
        result = response.json()
        assert result["code"] == 0
...
class TestDepartmentEndpoints:
    async def test_get_dept_tree(self, client: AsyncClient, db_session: AsyncSession):
        auth = await _superuser_bearer(db_session, "su_dept_tree")
        response = await client.get("/api/system/dept/tree", headers={"Authorization": auth})
        assert response.status_code == 200
        result = response.json()
        assert result["code"] == 0
    ...
    async def test_update_dept(self, client: AsyncClient, db_session: AsyncSession):
        ...
        response = await client.post(
            "/api/system/dept/create",
            headers={"Authorization": auth},
            json={"name": "更新部门", "parentId": None, "sort": 100, "status": 1},
        )
        dept_id = response.json()["data"]["id"]
        await db_session.commit()

        response = await client.put(
            f"/api/system/dept/{dept_id}", headers={"Authorization": auth}, json={"name": "已更新部门"}
        )
        assert response.status_code == 200
        result = response.json()
        assert result["code"] == 0
    ...
    async def test_delete_dept(self, client: AsyncClient, db_session: AsyncSession):
        ...
        response = await client.post(
            "/api/system/dept/create",
            headers={"Authorization": auth},
            json={"name": "待删除部门", "parentId": None, "sort": 100, "status": 1},
        )
        dept_id = response.json()["data"]["id"]
        await db_session.commit()

        response = await client.delete(f"/api/system/dept/{dept_id}", headers={"Authorization": auth})
        assert response.status_code == 200
        assert response.json()["code"] == 0
...
class TestIPRuleEndpoints:
    async def test_get_ip_rules(self, client: AsyncClient, db_session: AsyncSession):
        ...
        response = await client.post(
            "/api/system/ip-rule", headers={"Authorization": auth}, json={"pageNum": 1, "pageSize": 10}
        )
        assert response.status_code == 200
        result = response.json()
        assert result["code"] == 0

    async def test_create_ip_rule(self, client: AsyncClient, db_session: AsyncSession):
        auth = await _superuser_bearer(db_session, "su_ip_rule_create")
        response = await client.post(
            "/api/system/ip-rule/create",
            headers={"Authorization": auth},
            json={"ipAddress": "192.168.1.100", "ruleType": "blacklist", "reason": "测试黑名单", "isActive": 1},
        )
        assert response.status_code == 200
        result = response.json()
        assert result["code"] == 201
    ...
    async def test_get_ip_rule_detail(self, client: AsyncClient, db_session: AsyncSession):
        auth = await _superuser_bearer(db_session, "su_ip_rule_detail")
        response = await client.post(
            "/api/system/ip-rule/create",
            headers={"Authorization": auth},
            json={"ipAddress": "192.168.1.101", "ruleType": "whitelist", "reason": "测试白名单", "isActive": 1},
        )
        rule_id = response.json()["data"]["id"]
        await db_session.commit()

        response = await client.get(f"/api/system/ip-rule/{rule_id}", headers={"Authorization": auth})
        assert response.status_code == 200
        result = response.json()
        assert result["code"] == 0
        assert result["data"]["ipAddress"] == "192.168.1.101"

    async def test_update_ip_rule(self, client: AsyncClient, db_session: AsyncSession):
        auth = await _superuser_bearer(db_session, "su_ip_rule_update")
        response = await client.post(
            "/api/system/ip-rule/create",
            headers={"Authorization": auth},
            json={"ipAddress": "192.168.1.102", "ruleType": "blacklist", "reason": "原原因", "isActive": 1},
        )
        rule_id = response.json()["data"]["id"]
        await db_session.commit()

        response = await client.put(
            f"/api/system/ip-rule/{rule_id}", headers={"Authorization": auth}, json={"reason": "更新后的原因"}
        )
        assert response.status_code == 200
        result = response.json()
        assert result["code"] == 0

    async def test_delete_ip_rule(self, client: AsyncClient, db_session: AsyncSession):
        auth = await _superuser_bearer(db_session, "su_ip_rule_delete")
        response = await client.post(
            "/api/system/ip-rule/create",
            headers={"Authorization": auth},
            json={"ipAddress": "192.168.1.103", "ruleType": "blacklist", "reason": "待删除", "isActive": 1},
        )
        rule_id = response.json()["data"]["id"]
        await db_session.commit()

        response = await client.delete(f"/api/system/ip-rule/{rule_id}", headers={"Authorization": auth})
        assert response.status_code == 200
        assert response.json()["code"] == 0

    async def test_batch_delete_ip_rules(self, client: AsyncClient, db_session: AsyncSession):
        ...
        for i in range(3):
            response = await client.post(
                "/api/system/ip-rule/create",
                headers={"Authorization": auth},
                json={"ipAddress": f"192.168.1.{200 + i}", "ruleType": "blacklist", "isActive": 1},
            )
            ids.append(response.json()["data"]["id"])
        await db_session.commit()

        response = await client.post(
            "/api/system/ip-rule/batch-delete", headers={"Authorization": auth}, json={"ids": ids}
        )
        assert response.status_code == 200
        result = response.json()
        assert result["code"] == 0
