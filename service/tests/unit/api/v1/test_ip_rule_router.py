...
class TestIPRuleRouter:
    ...
    def test_create_ip_rule(self, client):
        resp = client.post(
            ...
            json={"ipAddress": "172.16.0.1", "ruleType": "blacklist", "isActive": 1},
            headers=self.auth,
        )
        assert resp.status_code == 200
    ...
    def test_create_ip_rule_with_valid_expires_at(self, client):
        ...
        resp = client.post(
            "/api/system/ip-rule/create",
            json={"ipAddress": "8.8.8.8", "expiresAt": "2030-01-01T00:00:00"},
            headers=self.auth,
        )
        assert resp.status_code == 200
        assert resp.json()["code"] == 201
    ...
    def test_create_ip_rule_with_invalid_expires_at(self, client):
        ...
        resp = client.post(
            "/api/system/ip-rule/create", json={"ipAddress": "9.9.9.9", "expiresAt": "not-a-date"}, headers=self.auth
        )
        assert resp.status_code == 422

    def test_update_ip_rule_with_valid_expires_at(self, client):
        """Update with valid ISO expiresAt should hit L122-123."""
        resp = client.put("/api/system/ip-rule/r1", json={"expiresAt": "2030-06-15T12:00:00"}, headers=self.auth)
        assert resp.status_code == 200
        assert resp.json()["message"] == "更新成功"
    ...
    def test_update_ip_rule_with_invalid_expires_at(self, client):
        ...
        resp = client.put("/api/system/ip-rule/r1", json={"expiresAt": "bad-date"}, headers=self.auth)
        assert resp.status_code == 422
